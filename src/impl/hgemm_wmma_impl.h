...
#define GET_HGEMM_F16_WMMA_M16N8K16_IMPL_NAME(BLOCK_M, BLOCK_N, BLOCK_K, BLOCK_M_WARPS, BLOCK_N_WARPS, WARP_SIZE, STAGES) \
    hgemm_f16_wmma_m16n8k16_##BLOCK_M##x##BLOCK_N##x##BLOCK_K##_w##BLOCK_M_WARPS##x##BLOCK_N_WARPS##x##WARP_SIZE##_s##STAGES##_

#define REGISTER_HGEMM_F16_WMMA_M16N8K16_IMPL(BLOCK_M, BLOCK_N, BLOCK_K, BLOCK_M_WARPS, BLOCK_N_WARPS, WARP_SIZE, STAGES)   \
    void GET_HGEMM_F16_WMMA_M16N8K16_IMPL_NAME(BLOCK_M, BLOCK_N, BLOCK_K, BLOCK_M_WARPS, BLOCK_N_WARPS, WARP_SIZE, STAGES)( \
        __half * c, const __half *a, const __half *b, const int m, const int n, const int k, gpuStream_t stream) {          \
        constexpr int VEC_SIZE = 16 / sizeof(__half);                                                                       \
        assert(n % VEC_SIZE == 0);                                                                                          \
        assert(k % VEC_SIZE == 0);                                                                                          \
        ...
        using WMMAT = WMMA_M16N8K16<__half, float>;                                                                         \
        hgemm_kernel<__half, WMMAT, WARP_SIZE, BLOCK_K, BLOCK_M_WARPS, BLOCK_N_WARPS, WARP_M_STEPS, WARP_N_STEPS,           \
                     STAGES><<<grid, block, 0, stream>>>(c, a, b, m, n, k);                                                 \
    }
