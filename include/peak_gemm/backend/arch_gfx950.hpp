...
using i16x4_t = short __attribute__((__vector_size__(4 * sizeof(short))));

PEAKGEMM_DEVICE PEAKGEMM_FORCEINLINE floatx4_t mfma_f32_16x16x16_f16_agpr(
    fp16x4_t &lhs,
    fp16x4_t &rhs,
...
PEAKGEMM_DEVICE PEAKGEMM_FORCEINLINE floatx4_t mfma_f32_16x16x16_bf16_agpr(
    bf16x4_t &lhs,
    bf16x4_t &rhs,
    floatx4_t &accumulator) {
    asm volatile(
...
struct MfmaM16N16K16 {
    static constexpr uint32_t m = 16, n = 16, k = 16;
    enum : uint32_t { M = m,
                      N = n,
                      K = k };
    ...
    PEAKGEMM_DEVICE PEAKGEMM_FORCEINLINE void operator()(
        ...
        if constexpr (std::is_same_v<Scalar, __half>) {
            if constexpr (UseAgpr) {
                mfma_f32_16x16x16_f16_agpr(
                    *reinterpret_cast<fp16x4_t *>(a),
                    *reinterpret_cast<fp16x4_t *>(b),
            ...
            mfma_f32_16x16x16_bf16_agpr(
                *reinterpret_cast<bf16x4_t *>(a),
                *reinterpret_cast<bf16x4_t *>(b),
                *reinterpret_cast<floatx4_t *>(const_cast<Accumulator *>(c)));
        } else {
            auto *d = reinterpret_cast<Accumulator *>(&destination);
            *reinterpret_cast<floatx4_t *>(d) =
                __builtin_amdgcn_mfma_f32_16x16x16bf16_1k(
                    *reinterpret_cast<const i16x4_t *>(a),
                    *reinterpret_cast<const i16x4_t *>(b),
                    *reinterpret_cast<const floatx4_t *>(c), 0, 0, 0);
        }
    }
    ...
    PEAKGEMM_DEVICE PEAKGEMM_FORCEINLINE void load_matrix(
        ...
        const auto offset = swizzle(target_row * stride + column + lane_ / 16 * 4);
