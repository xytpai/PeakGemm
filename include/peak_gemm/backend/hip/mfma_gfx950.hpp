...
struct MfmaM16N16K32 {
    ...
    static constexpr std::uint32_t k = 32;
        ...
        if constexpr (std::is_same_v<Scalar, __half>) {
            *reinterpret_cast<floatx4_t *>(d) =
                __builtin_amdgcn_mfma_f32_16x16x32_f16(
                    ...
                    *reinterpret_cast<const floatx4_t *>(c),
                    0,
                    0,
                    0);
        } else {
    ...
    PEAKGEMM_DEVICE PEAKGEMM_FORCEINLINE std::uint32_t swizzle(
        ...
        const auto column_bytes = column * sizeof(Scalar);
        return (column_bytes ^ ((row % KBlocks16) * 16)) / sizeof(Scalar);
    }
    ...
    PEAKGEMM_DEVICE PEAKGEMM_FORCEINLINE void load_matrix(
        ...
        const auto y = lane_ % 16;
        const auto x = lane_ / 16 * 8;
        const auto target_row = row + y;
        auto target_column = column + x;
        if constexpr (UseSwizzle) {
            target_column = swizzle(target_row, target_column);
        }
...
#pragma unroll
        for (std::uint32_t index = 0; index < 4; ++index) {
            pointer[(y + index) * stride + x] =
                static_cast<Scalar>(fragment.values[index]);
        }
    }
