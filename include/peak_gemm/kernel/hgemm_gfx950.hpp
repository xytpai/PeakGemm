...
template <typename scalar_t, typename wmma_t, typename swizzle_t, uint32_t BLOCK_K, uint32_t BLOCK_M_WARPS, uint32_t BLOCK_N_WARPS, uint32_t WARP_M_STEPS,
          uint32_t WARP_N_STEPS, uint32_t STAGES, bool HAS_BIAS, bool IS_SPLIT_K>
...
template <uint32_t STAGES, uint32_t COPY_INSTRUCTIONS, uint32_t STAGE, typename block_tile_t>
PEAKGEMM_DEVICE_INLINE void run_epilogue_stages(block_tile_t &block_tile, uint32_t &current_stage) {
    if constexpr (STAGE < STAGES - 1) {
        ...
        run_epilogue_stages<STAGES, COPY_INSTRUCTIONS, STAGE + 1>(block_tile, current_stage);
    }
}
...
hgemm_kernel(scalar_t *c, const scalar_t *a, const scalar_t *b, uint32_t m_size, uint32_t n_size, uint32_t k_size, uint32_t split_k, uint32_t *semaphore,
...
#pragma unroll
    ...
    run_epilogue_stages<STAGES, COPY_INSTRUCTIONS, 0>(block_tile, current_stage);
